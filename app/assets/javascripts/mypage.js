...
var mymap = L.map('mapid').setView(default_latlng, 10);
...
gon.events.forEach((event) => {
  ...
  var content = `<a href="/events/${event.event.id}">${event.event.title}</a><div>${event.date}<div><div>${category}</div><div><a href="events/${event.id}/edit">編集する</div>`;
